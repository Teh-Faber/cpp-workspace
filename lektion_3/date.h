  ...
  public:
    Date(int x){
       //initialize _x to input
        _x = x;
       //check validity of date, set to firstDay if it isnt
        ...
        do {
         ...
         //for optimization
             ...
             //if month exceeds 12, move to the next year
              if (getMonth() > 12)
                 _x += 10000 - ((getMonth()+1) * 100); //move to the 1st month of next year
        } while(!isValid());
      return;
    }
    ...
    int differenceInDays(Date future){
     //find the difference in days by incrementing for every valid day in between
      int i     = 0;
      int og_x  = _x;     //save current date since _x will be overwritten by incrementDate

       //check for which date is greatest. If neither then its equal and return 0
        if(_x < future._x){
    ...
    int dayOfTheYear(){
     //find day of the year with differenceInDays from Jan 1st
        Date currentDate(_x);           //save current date
        _x = getYear()*10000 + 0101;    //set to Jan 1st of current year

        int res = differenceInDays(currentDate);

        _x = currentDate._x;            //restore _x
      return res;
    }
